...
def plot_result(*args):
    ...
    name_1st = ['Image{}'.format(i) for i in range(1, number_images)]
    name_1st.append('Result')
    for ax, name, image in zip(axis, name_1st, args):
        ax.set_title(name)
...
def plot_histogram(image):
    fig, axis = plt.subplots(nwors=1, ncols=3, figsize=(
